...
-   If in Africa: $$\widehat{LE} = `r round(means_LE$mean[1], 2)`$$

-   If in the Americas: $$\widehat{LE} = `r round(means_LE$mean[2], 2)`$$

-   If in Asia: $$\widehat{LE} = `r round(means_LE$mean[3], 2)`$$

-   If in Europe: $$\widehat{LE} = `r round(means_LE$mean[4], 2)`$$
...
#| echo: false
ggplot(gapm2, aes(x = FoodSupplykcPPD, y = LifeExpectancyYrs, 
                  color = income_levels)) +
  geom_point() +
  geom_smooth(method = lm, se = FALSE) + 
  labs(x = "Food supply (kcal PPD)", 
       y = "Country life expectancy (years)",
       title = "Life expectancy vs. Food supply", 
       color = "Income levels") +
  theme(axis.title = element_text(size = 20), 
        axis.text = element_text(size = 20), 
        title = element_text(size = 20))
